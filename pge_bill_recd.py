...
def main():
    ...
    for acct in acct_list:
        ...
        logger.trace(f'current_usage_percent (b4 calculation): {acct.current_usage_percent}')
        logger.trace(f'current_usage_percent: {(acct.current_usage / total_usage) * 100}')
        logger.trace(f'total_usage: {total_usage}')

        acct.current_usage_percent = round((acct.current_usage / total_usage) * 100, 2)
        logger.trace(f'current_usage_percent (rounded): {acct.current_usage_percent:.2f}')
        logger.trace(f'pge_bill: {int(pge_bill)}')
        ...
        if savings_balance < 1000000:
            ...
            logger.trace(f'Bill total: {int(round(acct.savings_assessment + acct.pge_bill_share, 2))}')
        else:
